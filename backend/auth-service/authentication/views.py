...
def user_list_create(request):
    """
    Endpoint: Create user or list users
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return Response(
                {'error': f'Ya existe un usuario con el email: {e.email}'},
        ...
        try:
            ...
            result = use_case.execute(current_user, include_inactive)

            response_serializer = UserResponseSerializer([r.__dict__ for r in result], many=True)
...
def user_detail(request, user_id):
    """
    Endpoint: Get, update or deactivate an user
    ...
    if request.method == 'GET':
        try: 
            ...
            if not current_user.can_manage_users():
                return Response(
                    {
                        'error': f'El rol {current_user.role.to_spanish()} no puede ver usuarios'
                    },
                    status=status.HTTP_403_FORBIDDEN
                )
            ...
            if not user:
                return Response(
                    {'error': f'Usuario no encontrado: {user_id}'},
                    status=status.HTTP_404_NOT_FOUND
                )

            serializer = UserResponseSerializer(UserResponseDTO.from_entity(user).__dict__)
